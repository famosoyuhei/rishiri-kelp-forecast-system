...
- **実装ファイル**: `start.py` `get_forecast()` 内、約L847-L863
  （`terrain_database.py` / `konbu_flask_final.py` は存在しない旧参照 — 削除済み）

| 補正項目 | 現行実装（簡易版） | 目標実装（改善予定） |
|---------|-----------------|------------------|
| **森林・風速** | −2.5 m/s（加算、常時） | ×0.4（乗算） |
| **森林・湿度** | +10%（固定、常時） | 季節・日射依存の水蒸気圧補正 |
| **海岸・風速** | +1.0 m/s（常時） | +1.0〜×1.25（onshore風向時のみ） |
| **海岸・湿度** | +5%（常時） | onshore方向時のみ |
| **標高・湿度** | −1%/100m | 現行維持 |
| **霧リスク** | 未実装 | `(T − Td) < 2°C` → 乾燥スコア減算（dewpointデータ取得済み） |
| **フロード数** | 未実装 | Fr=U/(Nh)で越山/回り込み体制を切り替え（850/700hPaデータ取得済み） |
| **山背風ボーナス** | 未実装（表示のみ） | `wind_mountain_angle_diff > 150°` かつ 風速>3 m/s → スコア+10〜15点 |
| **カタバ風ボーナス** | 未実装 | 早朝01:30通知用: 雲量<20% + 地上風<3 m/s → 加点 |

> 各改善の気象学的根拠は [ISLAND_METEOROLOGY_RESEARCH.md](ISLAND_METEOROLOGY_RESEARCH.md) §2〜§7 を参照。
...
#### 科学的乾燥モデル（研究文献準拠）
- **モデル**: Pageモデル MR = exp(−k × t^n)（Laminaria japonica実測 R²=0.873〜0.999）
- **有効水分拡散係数**: Deff = 1.00〜13.00×10⁻¹¹ m²/s（日射≥400 W/m²で最大7倍向上）
- **乾燥3段階**: 恒率乾燥期（表面蒸発）→ 減率乾燥期（内部拡散律速）→ 平衡（臨界含水率~55%）
- **閾値の物理的根拠**:
  - 湿度94%: 水蒸気圧勾配≈0で蒸発停止（水活性理論）
  - 風速2.0 m/s: 大気境界層薄化で質量移動係数が急増
- **詳細**: [KOMBU_DRYING_RESEARCH.md](KOMBU_DRYING_RESEARCH.md)
- **現行実装**: `start.py`内の予報スコア計算（kelp_drying_model.pyは存在しない）
...
干場記録は不特定多数ユーザーが共有する「1干場1日1記録」方式。記録者の同一性は公開しない。

### 記録フロー

1. 干場を選択し「📝 記録を入力」ボタンをクリック
2. 日付を選択（デフォルト: 当日）
3. 日付変更時に自動で既存記録を非同期チェック
   - **記録なし**: ① 乾燥結果選択へ進む
   - **記録あり**: 「既に他のユーザーによって以下の記録が行われています（記録日時・訂正回数を表示。記録者名は非公開）」と表示し、「戻る」か「記録を訂正する」の2択

### 段階式入力フォーム（v2.6.0 新設計）

#### ① 乾燥結果（必須、4択）

| 選択肢 | 色 | 意味 |
|--------|-----|------|
| 完全乾燥 | 緑 | 目標含水率まで完全に乾燥できた |
| 概ね乾燥 | 黄緑 | ほぼ乾燥したが若干残った |
| 半乾燥 | 黄 | 半分程度しか乾燥できなかった |
| ほぼ乾燥なし | 赤 | ほとんど乾燥できなかった・干さなかった |

#### ② 中止・不調の主因（①が「概ね乾燥」「半乾燥」「ほぼ乾燥なし」のとき表示、5択）
...
- ①が「完全乾燥」の場合はこのステップをスキップ
- ①が「ほぼ乾燥なし」の場合、②の後に「干しましたか？」サブ選択（干した / 干さなかった）が現れる

#### ③ 取り込み時刻（①が「完全乾燥」「概ね乾燥」「半乾燥」のとき、または「ほぼ乾燥なし＋干した」のとき表示）

- HH:MM 形式の時刻入力（任意）
- 「ほぼ乾燥なし」かつ「干さなかった」の場合はスキップ
...
### CSVデータ形式（hoshiba_records.csv）

| カラム | 型 | 説明 |
|--------|-----|------|
| date | YYYY-MM-DD | 記録対象日 |
| name | str | 干場ID（H_XXXX_XXXX） |
| result | str | 乾燥結果（4択） |
| stop_cause | str or null | 中止・不調主因（5択、①が完全乾燥なら空） |
| did_dry | str or null | 「干した」「干さなかった」（①がほぼ乾燥なしのみ） |
| collection_time | HH:MM or null | 取り込み時刻（任意） |
| recorded_at | ISO8601 JST | 記録確定日時 |
| correction_count | int | 訂正された回数（初回=0） |
| correction_reason | str or null | 訂正理由（訂正時のみ） |

#### 有効な result 値
- `"完全乾燥"`, `"概ね乾燥"`, `"半乾燥"`, `"ほぼ乾燥なし"`

#### 有効な stop_cause 値
- `"雨が降った"`, `"霧が出た・昆布が湿り戻った"`, `"飛散リスク・強風"`, `"曇り続きで乾かなかった"`, `"天候以外"`, `null`

### 記録機能UI実装詳細（v2.6.0 再設計済み）

#### ポップアップ記録フォーム
- **アクセス方法**: 干場マーカーのポップアップメニューから「📝 記録を入力」ボタンをクリック
- **表示形式**: 地図左上にモーダル形式のフォームが表示
- **デフォルト日付**: 当日の日付が自動設定
- **既存記録表示**: 既存記録がある場合、記録日時と訂正回数を表示（記録者名は非公開）
- **匿名性**: 記録者の個人情報は一切保存・表示しない

#### API仕様
- **記録追加/更新**: `POST /record` - JSON形式で name, date, result, stop_cause, did_dry, collection_time, correction_reason を送信
- **記録確認**: `GET /record/{name}/{date}` - 既存記録の有無・内容・recorded_at・correction_count を取得
- **データ形式**: CSV形式で `hoshiba_records.csv` に保存（9カラム）

---

## 予報精度フィードバックシステム（v2.6.1 実装完了）✅

### 概要

乾燥記録が追加・訂正されるたびに、その日の予報判定と実結果を自動照合し、`feedback_log.csv` に蓄積する。
...
_record_forecast_feedback(name, date, result)
  ↓
forecast_history/{name}/forecast_*_for_{YYYYMMDD}.json を検索
  ↓
予報 suitability(excellent/good → "可" / fair/poor → "不可")
実記録(完全乾燥/概ね乾燥 → "可" / 半乾燥/ほぼ乾燥なし → "不可")
  ↓
二値ラベルを比較 → judgment_correct (True/False)
  ↓
feedback_log.csv に追記（記録訂正時は対象行を上書き）
```

### feedback_log.csv カラム定義

| カラム | 型 | 説明 |
|--------|-----|------|
| date | YYYY-MM-DD | 対象日 |
| spot_name | str | 干場ID |
| actual_result | str | 実記録（4択） |
| actual_label | "可"/"不可" | 二値化した実結果 |
| forecast_score | int | 予報スコア（0〜100） |
| forecast_suitability | str | 予報判定（excellent/good/fair/poor） |
| forecast_label | "可"/"不可" | 二値化した予報判定 |
| judgment_correct | bool | 予報が当たったか |
| days_ahead | int | 何日前の予報か（1〜6） |
| recorded_at | ISO8601 JST | フィードバック記録日時 |

### 精度指標（/api/validation/accuracy の返却値）

従来の数値誤差（MAE）に加え、以下の判定精度指標を追加：

| 指標 | 説明 |
|------|------|
| `judgment_accuracy_by_day` | 何日前の予報ごとの判定正答率・TP/FP/FN/TN |
| `judgment_accuracy_overall` | 全日数統合の正答率 |
...
| forecast suitability | forecast_label |
|---------------------|---------------|
| excellent / good | "可" |
| fair / poor | "不可" |
...
- **`start.py`**: `_result_to_label()`, `_suitability_to_label()`, `_record_forecast_feedback()` 関数
- **`feedback_log.csv`**: 自動生成（初回記録時に作成）
- **`forecast_history/`**: `_save_forecast_history()` が予報発表時に保存（既存）
- **`/api/validation/accuracy`**: `judgment_accuracy_by_day` / `judgment_accuracy_overall` フィールドを追加返却

### 注意事項

- 予報履歴（`forecast_history/`）がない日のフィードバックは記録されない（初回起動直後の記録等）
- 記録訂正時は `feedback_log.csv` の該当行も自動上書きされる
- フィードバック記録の失敗は記録保存本体には影響しない（try/except で分離）
...
- **実行時刻**: 毎日03:00 JST（バックグラウンドスレッド、前日分を取得）
- **収集データ**: 1日分の時別データ（Open-Meteo Archive API経由）
  - 気温、湿度、風速、降水量
- **保存先**: `amedas_data/amedas_{station_id}_YYYYMMDD.json`
  - 沓形（11151）: `amedas_data/amedas_11151_YYYYMMDD.json`
  - 本泊（11311）: `amedas_data/amedas_11311_YYYYMMDD.json`
- **実装ファイル**: `start.py` 内 `_collect_amedas_from_openmeteo()` / `_daily_amedas_collection()`
  （`amedas_auto_fetcher.py` は存在しない旧参照 — 削除済み）
- **手動バックフィル**: `GET /api/collect_amedas?days=N` で過去N日分を一括取得可能
- **データソース**: Open-Meteo Archive API（`archive-api.open-meteo.com/v1/archive`）
  JMA AMeDAS直接APIは404エラーが多いため、Open-Meteoに統一
- **重要**: 収集スレッドはJSTで03:00に起動するよう実装済み

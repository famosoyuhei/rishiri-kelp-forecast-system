...
| 15 | 🟠 MAJOR候補 | フェーン多変数整合性監査担当（ChatGPT再検証で新規発見） | `_get_summit_hourly_temps()`の30分キャッシュ(`_field_cache_get/set`)と、干場側の毎回フレッシュ取得の間で、Open-Meteoのモデル実行世代がズレるタイミングが理論上あり得る（`/api/forecast`経路のみ。`_compute_score_field()`は全点を同一並列取得のため対象外）。`theta_spot - theta_summit`が異なる実行世代同士の比較になり得る | 2026-08-04 | **📊 診断ログ実運用化済(2026-08-05)** 環境変数`FOEHN_DIAGNOSTICS_ENABLED=true`をRenderに設定すると`rishiri_kelp.foehn_diagnostics`専用ロガー（app.loggerと独立、propagate=False）からINFOログが出力される。未設定時は従来どおり抑制、欠損WARNINGは常時出力。ハンドラ多重追加防止済み。予報値・キャッシュ動作・判定結果への影響なしを再確認。ロジック自体の修正（C項の影計測含む）は計測結果待ちで未着手 |
| 16 | 🔴 CRITICAL（本番障害） | Eの観測作業中に発見 | `_fc_redis_set()`のUpstash REST書き込みが`POST /set/<key>` + body`["EX", ttl, payload]`という誤った形式で、Upstashが配列全体をそのまま値として保存していた。結果、`_field_cache_get()`のRedis経由キャッシュヒットが常に破損データ（例: `['EX', 1800, '{"time":...}']`）を返し、2026-08-05に`FOEHN_DIAGNOSTICS_ENABLED=true`設定→再デプロイ直後、`/api/forecast`が全滅（500→ホットフィックスで503→今回の根本修正で解消）。ログ調査で`/api/analysis/field`（`get_analysis_field()`, L5228付近）でも**今回の変更と無関係な時刻(02:54, 03:09)**に同じ原因の500が既に発生しており、`_field_cache_get/set`を使う既存キャッシュ全般が前から間欠的に壊れていた既存バグと判明 | 2026-08-05 | **コード修正はデプロイ済み(commit fb77464, 2026-08-05)、本番検証は一部保留 🟡**。`_fc_redis_set()`をUpstash REST root endpointへの`["SET", key, payload, "EX", ttl]`（Redis本来の引数順）に修正し、`{"result":"OK"}`確認を追加。`_FC_KEY_PREFIX`を`fc:`→`fc2:`にバージョンアップし旧破損データを遮断。`_field_cache_get()`にdict型必須の共通ガードを追加。`get_analysis_field()`のキャッシュヒット処理を copy-then-mutate に修正。`tests/test_field_cache.py`(15件、HTTP mock で送信JSON配列を直接検証)新規追加、既存202件+新規15件全パス。デプロイ後`malformed value`警告は再発なし（修正は効いている兆候）。**ただし検証者自身の連続テストによりOpen-MeteoがRenderサーバーIPを429でレート制限しており、8項目の本番検証（/api/forecast・/api/analysis/fieldの200連続確認、キャッシュミス/ヒット値の一致等）はまだ完了と扱わない**。429解消後、間隔を空けた単発確認（/api/forecast×2、/api/analysis/field×2、計4回程度）で再開する。詳細は項目17参照 |
| 17 | 🟠 MAJOR候補（運用リスク） | 項目16のデプロイ検証中に発見 | 2026-08-05、根本修正のデプロイ後検証中、**Open-MeteoがRenderサーバーIPを429（Too Many Requests）でレート制限**。ローカルPCから直接Open-Meteoを叩くと200成功するため、Render側IP限定の制限と確認。原因は本日のデバッグ作業の累積リクエスト量（項目16の破損キャッシュが直っていなかった期間、`_get_summit_hourly_temps()`が常にキャッシュミス扱いとなり毎回フレッシュ取得していたことも一因）。5分待機後も解消せず、時間単位クォータ枯渇の可能性。**根本的な構造リスク**: 331干場は個別に`/api/forecast`ごとにOpen-Meteoを都度取得する設計のため、無料APIの日次・時間上限に本質的に接近しやすい（`_compute_score_field()`の49点グリッドのみ`_fetch_open_meteo_multi()`で複数座標バッチ取得済み、個別干場予報は未対応） | 2026-08-05 | 未着手・調査継続中。**中期対応候補**（未実装、規模の大きい変更のため別タスク）: ①複数座標のバッチ取得（Open-Meteoは緯度経度カンマ区切りで複数地点を1リクエストで返せる。`_fetch_open_meteo_multi()`と同じ手法を`/api/forecast`にも適用）、②定期一括更新+Redisスナップショット方式（ユーザーリクエスト時にOpen-Meteoを都度叩くのではなく、バックグラウンドジョブが定期的に全干場分をまとめて取得しRedisに保存、APIは保存済みデータを返すだけにする）、③Open-Meteo有料プラン検討。**短期対応候補**（未実装）: 429発生時にTTL切れ後もキャッシュを保持し「stale-if-error」的にフォールバック提供する設計（フィージビリティのみ検討、大規模変更のため今回はデプロイしない — 実装時は最大許容staleness・レスポンスへの「古いデータ」明示・`/api/forecast`と`/api/analysis/field`双方への統一適用が必要） |
...
### 2026-07-22（フェーン検知の06時アンカー化・地形断面プロトタイプ・世界の技術調査）

**背景**: 前日の`FOEHN_PHYSICS_AUDIT_20260721.md`で「弱い風下日でも補正が効きすぎる」問題を発見した続き。

**実施内容**（すべてスクリプトでの検証のみ、`start.py`は未変更）:
1. **06時アンカー方式**: 代表風向を04-16時平均→06時単独の実測値に変更（ユーザー指摘: 昼は谷風・海風収束のノイズが乗る）。50日バックテストで①風上完全不変②風下スコア50/50日で上昇（平均+11.5点）③風下日射50/50日で上昇（平均+86W/m²）④気温96%の日で風下高温⑤湿度84%の日で風下低湿度、を確認。
2. **山頂座標の誤りを発見**: `start.py`のSUMMIT定数(45.1800,141.2392)はGSI標高で1551.2mしかなく、JMA公式の山頂座標(45.1786,141.2419, 45°10'43"N 141°14'31"E)では1700.2m（文書値1721mとほぼ一致）。約260m水平にズレていた。
3. **地形断面（streamline effective barrier height）プロトタイプ**: 各地点の06時風上方向に地形をサンプリングし、実際に超える標高を使う設計。Open-Meteo Elevation APIの1日クォータを使い果たし（複数回のリトライ試行錯誤が原因）、国土地理院(GSI)標高API（1mレーザー、Rishiri全域カバー確認済み）に切替。ただし2kmグリッド解像度では山頂を1,246mまでしか捉えられず、かつ実装コストが高い割に「弱い日/強い日」比例性の改善は限定的（0.76→0.65、地形なし角度改良のみでも0.69まで到達）と判明。
4. **山頂(R_1800_2392)を精密アンカーとして注入**する案を検証 → 地形の絶対精度は上がるが（194/2400地点日で発動）、比例性は逆にやや悪化（0.65→0.69）。地形は「地理的に静的」なため日々の大気の強弱とは無関係、という理由を特定。
5. **コスト対効果の結論**: 山頂座標修正＋`_leeward_intensity()`の角度ランプ改良（不感帯90°→100°、二乗ランプ化）が最も安価（`start.py`定数・数式数行、外部依存なし）で、比例性0.76→0.69まで改善。地形断面は「ついで」の追加候補（データ取得済みでコストほぼゼロだが主目的の解決にはならない）。
6. **世界の技術調査**（`AGENT_foehn_parameterization_research.md`新設）: MeteoSwissの客観的フェーン指数（Dürr 2008、2008年から運用中）が**山頂リファレンス観測点との温位差**で判定する方式と判明。本システムには既に山頂特別地点(R_1800_2392)の予報インフラがあるため、幾何学的な角度×風速ランプより**実測（予報）ベースの山頂-地点差分**の方が精度・コストともに優れる可能性が高いと結論。詳細は`FOEHN_PARAMETERIZATION_RESEARCH_20260722.md`。

**訂正**: 本ログ執筆時点では「PR #18は既にマージ済み」と誤認していたが、実際は**未マージ（OPENのまま）**だった。実装着手時に発覚し、PR #18の内容（連続値検知・50%上限・15点上限）を本セッション最終PRに統合してPR #18はクローズする方針に変更（下記「実装完了」参照）。

**実装完了（同日 2026-07-22 続き）**:
MeteoSwiss式（山頂温位差ベース）を`start.py`に実装。詳細:
- `_compute_foehn_intensity_hours()`新規実装（旧`_leeward_intensity()`/`_compute_foehn_hours()`置き換え）。06時角度ゲート(>90°)＋山頂(R_1800_2392基準、JMA公式座標45.1786/141.2419に修正)との温位差でスケーリング
- `_get_summit_hourly_temps()`新規実装。`_field_cache_get/set()`流用でRedis+インメモリ30分キャッシュ、新規APIコールは実質1回/30分
- `_apply_leeward_solar_boost()` 30%→50%、`_apply_local_risk_adjustments()`フェーンボーナス 8pt→15pt（stage_analysis側と統一）
- 較正定数はキャッシュ済み50日データで再計算（山頂標高1721m定数を反映）: LOW=-6.65, HIGH=-5.266
- 検証: 風上完全不変（差分0.0）、比例性0.76→0.518、`/api/forecast`・`/api/analysis/field`ともローカル起動で実地確認済み、テスト17件全パス
- PR #18はこのPRに統合されるためクローズ（マージしない）
- ブランチ: `feature/foehn-meteoswiss-index`（`main`＝PR#17マージ後の状態から分岐）

---

### 2026-07-21（風下側フェーン補正の実装・バックテスト・物理監査）

**背景**: 漁業者フィードバック「南西風で鴛泊、北東風で沓形が、曇り予報でも実際は晴れて乾く」に対応。

**実施内容**:
1. **PR #17**: `_apply_leeward_solar_boost()` 新規実装。フェーン判定時間の比率でスコア入力用日射量を最大+30%補正（マージ済み）
2. **50日バックテスト（AMeDAS2点）**で反転日数=0件を発見 → **PR #18**: フェーン検知を離散AND条件（角度差>150°かつ風速>3m/s毎時間同時）から連続値 `_leeward_intensity()` に変更、倍率を30%→50%、フェーンボーナス上限8→15点に引き上げ（→2026-07-22時点で未マージと判明。2026-07-22実装で内容ごと統合、PR #18はクローズ）
3. ユーザー指摘「代表風向は06:00の実測値のみを使うべき（昼は谷風・海風収束のノイズ）」→ **スクリプト側のみ**06時アンカー方式で再シミュレーション（`start.py`は未変更）
   - 結果: 風上完全不変、風下スコア50/50日で上昇（平均+11.5点）、日射50/50日で上昇（平均+86W/m²）、気温96%の日で風下高温（平均+0.41℃）、湿度84%の日で風下低湿度（平均-1.12%）
4. **`ai_review_agents/AGENT_foehn_physics_audit.md` 新設**: 新潟・浜松のフェーン実測データ（富山+7.9℃、教科書2500m越え+7.2℃、浜松41.1℃の伊吹山地1,377m要因等）と比較する専任AI社員を追加
5. **`FOEHN_PHYSICS_AUDIT_20260721.md`**: 気温差3分位で補正量を検証 → **弱い風下日（気温差+0.13℃、ノイズレベル）でも平均+9.81点というほぼ最大値近い補正が乗っており過大補正の疑い**。原因は `_leeward_intensity()` の角度ランプが90°から間髪入れず立ち上がる設計。修正案（不感帯の追加、二乗ランプ化）を提示済み、**未実装**

**次回やること**:
- `_leeward_intensity()` のランプ形状修正（90°起点の不感帯追加 or 二乗ランプ）を実装するか判断
- 06時アンカー方式を実際に `start.py` に反映するか判断（現状スクリプトのみ）
- 修正後は再度「気温差3分位 vs 補正量」の比例関係を検証してから本番反映

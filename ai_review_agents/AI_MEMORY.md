...
| 16 | 🔴 CRITICAL（本番障害） | Eの観測作業中に発見 | `_fc_redis_set()`のUpstash REST書き込みが`POST /set/<key>` + body`["EX", ttl, payload]`という誤った形式で、Upstashが配列全体をそのまま値として保存していた。結果、`_field_cache_get()`のRedis経由キャッシュヒットが常に破損データ（例: `['EX', 1800, '{"time":...}']`）を返し、2026-08-05に`FOEHN_DIAGNOSTICS_ENABLED=true`設定→再デプロイ直後、`/api/forecast`が全滅（500→ホットフィックスで503→今回の根本修正で解消）。ログ調査で`/api/analysis/field`（`get_analysis_field()`, L5228付近）でも**今回の変更と無関係な時刻(02:54, 03:09)**に同じ原因の500が既に発生しており、`_field_cache_get/set`を使う既存キャッシュ全般が前から間欠的に壊れていた既存バグと判明 | 2026-08-05 | **✅根本修正済(2026-08-05)** `_fc_redis_set()`をUpstash REST root endpointへの`["SET", key, payload, "EX", ttl]`（Redis本来の引数順）に修正し、`{"result":"OK"}`確認を追加。`_FC_KEY_PREFIX`を`fc:`→`fc2:`にバージョンアップし旧破損データを遮断。`_field_cache_get()`にdict型必須の共通ガードを追加（非dictは無条件でキャッシュミス扱い、in-memoryへフォールバック）。`get_analysis_field()`のキャッシュヒット処理を`cached['cache']=...`（共有オブジェクト直接変更）から copy-then-mutate に修正。`tests/test_field_cache.py`(15件、HTTP mock で送信JSON配列を直接検証)新規追加、既存202件+新規15件全パス |

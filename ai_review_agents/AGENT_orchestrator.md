...
18名分のチェックリストを順番にこなし、最後に統括プロンプトを生成してください。
...
```bash
# ファイル存在確認
ls ai_review_agents/AGENT_*.md | wc -l          # 18以上あればOK
...
確認後、「準備完了。18名のレビューを開始します。」と宣言してください。
...
### 社員16：💾 Upstash Redisデータ永続化担当

```bash
# URLの正規化確認（https://自動補完）
grep -n "def _upstash_url\|startswith.*http\|rstrip" line_integration.py | head -5

# _upstash_set がパイプライン形式か確認（ダブルエンコード排除）
grep -n "def _upstash_set\|/pipeline\|\[.*SET" line_integration.py | head -10

# _upstash_get のエラーハンドリング確認
grep -n "def _upstash_get\|status_code.*200\|isinstance.*dict" line_integration.py | head -10

# フォールバックログの確認（Upstash失敗時のWARNING）
grep -n "falling back\|Upstash NOT available\|lost on redeploy" line_integration.py | head -5

# 登録解除コマンドの削除ロジック確認
grep -n "def handle_remove_spot\|del nicknames\|spots.remove\|upsert_subscription" line_integration.py | head -10

# 診断エンドポイントの認証確認
grep -n "def get_debug\|admin_secret\|X-Notify-Secret\|unauthorized" line_integration.py | head -10

# デバッグ出力にトークンが含まれないか確認
grep -n "upstash_token\|UPSTASH_REDIS_REST_TOKEN" line_integration.py | grep -i "result\|return\|jsonify" | head -5
```

AGENT_upstash_persistence.md の全チェックリストに従って精査してください。
...
```bash
# JMA API定数・ステーションIDの確認
grep -n "JMA_AMEDAS_LATEST_URL\|RISHIRI_AMEDAS_STATIONS\|_AMEDAS_RT_CACHE_TTL" start.py

# キャッシュとエラーハンドリングの確認
grep -n "_fetch_jma_amedas_realtime\|_AMEDAS_RT_CACHE" start.py | head -15

# UIの実測値バー要素確認
grep -n "amedasRealtimeBar\|loadAmeDASRealtime" kelp_drying_map.html | head -10

# forecast_historyの蓄積状況（ディスク圧迫リスク）
find forecast_history -name "*.json" 2>/dev/null | wc -l

# feedback_log.csvの存在確認
ls -la feedback_log.csv 2>/dev/null || echo "feedback_log.csv なし（記録未入力）"
```

AGENT_amedas_accuracy.md の全チェックリストに従って精査してください。

---

### 社員18：🧮 精度分析スプレッドシート監査担当

```bash
# Sheets連携APIの存在確認
grep -n "validation/accuracy/sheets\|integration/spots/sheets" start.py

# API列とApps Scriptヘッダーの突合
...
# JST実行時刻と負荷回避
grep -n "Asia/Tokyo\|JST\|03:3\|03:4\|api/forecast\|334\|連続" docs/N8N_GOOGLE_SHEETS_SETUP_CHECKLIST.md docs/N8N_GOOGLE_SHEETS_ACCURACY.md

# Dashboard数式とspot_detail参照
grep -n "setFormula\|FILTER\|AVERAGE\|SUM\|requireValueInRange" docs/google_sheets_accuracy_dashboard_apps_script.gs
```

AGENT_accuracy_spreadsheet_auditor.md の全チェックリストに従って精査してください。実際のGoogle Sheets URLが提供されている場合は、シート上の重複・数式エラー・Dashboard表示まで確認してください。

---

## PHASE 2：進捗サマリーの表示

18名全員完了後、以下を出力してください：

```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
✅ PHASE 1 完了 — 18名のレビュー結果
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

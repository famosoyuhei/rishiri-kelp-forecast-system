...
def get_forecast_for_spot(lat: float, lon: float, timeout: int = 20) -> list:
    ...
    for i in range(min(7, len(daily.get('time', [])))):
        try:
            ...
            max_temp = (daily.get('temperature_2m_max') or [None] * 7)[i]

            # 干場稼働時間帯 04:00-16:00 JST（13時間）
            start_h = i * 24 + 4
            end_h = start_h + 13
            work_rh = [
                hourly_rh[h]
            ...
            work_pr = [
                hourly_pr[h]
                for h in range(start_h, min(end_h, len(hourly_pr)))
                if hourly_pr[h] is not None
            ]
            ...
            precip_0416 = round(sum(work_pr), 2) if work_pr else precip
            ...
            days.append({
                ...
                'precipitation_0416': precip_0416,
                'max_temp': max_temp,
...
「漁期開始 6/15」→ 通知の開始日を設定
「漁期終了 9/5」 → 通知の終了日を設定
「沖止め 6/25」  → その日の通知をスキップ
※ 設定しない場合、6/1〜9/30の間は毎日通知が届きます
